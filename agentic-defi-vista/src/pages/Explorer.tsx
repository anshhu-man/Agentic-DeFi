...
const Explorer = () => {
  ...
  useEffect(() => {
    setSearchResults(protocolData);
    setAiInsights(originalAiInsights);
  }, []);
    ...
    if (useRealData && healthData?.status === 'healthy') {
      ...
      try {
        ...
        if (response.success) {
          ...
          setSearchResults(transformedData.length > 0 ? transformedData : protocolData);
